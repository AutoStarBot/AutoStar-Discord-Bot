...
//Servers that are verifed as 'Good' servers
const verifedServers = ['','']
//Servers that are verifed as 'Bad' servers and the bot won't join
...
client.once('ready', async () => {
    ...
    return
    await client.user.setAvatar('./images/gif.gif').catch(async err => {
        error = true;
        console. log(err);
    });
});
...
client.on('guildCreate', (g) => {
    ...
    if (serversToLeave.includes(g.id)) {
        ...
        messageEmbed.setDescription("I have a list of channel names I wont react in but if you don't want me to react to messages in a channel, just make it so that I can't see the channel. \n If you dont want me to react to a message send message with (copy code box above) at the beginning")
...
client.on('interactionCreate', async interaction => {
        ...
        .then(newchannel => {
            ...
            .then(message => {
                ...
                if (newName.content === '' || newName.content === '‎'){
                    ...
                    messageEmbed.setDescription('(channel names cant be invisable). Command Ended/Cloned channel deleted')
                    interaction.editReply({ embeds: [messageEmbed] , ephemeral: true }); 
                    newchannel.delete(); 
                    return
                } 
    ...
    if (interaction.commandName === 'otherside'){
        ...
        }else if(interaction.channel.guildId === ''){
            guild = client.guilds.cache.get('');
        }else if(interaction.channel.guildId === ''){
            guild = client.guilds.cache.get('');
        }else if(interaction.channel.guildId === ''){
        ...
        if (!userMember.permissions.has(PermissionFlagsBits.ManageChannels)) {
            messageEmbed.setColor('Red')
            messageEmbed.setTitle('Error \'User Missing Permission\'')
            messageEmbed.setDescription('Only someone with Manage Channels permission can use this command!')
            interaction.reply({ embeds: [messageEmbed] , ephemeral: false });
        ...
        const secondLowestCategory = categories.size > 1 ? categories.at(categories.size - 3) : null;
        ...
        .then(newchannel => {
            ...
            const messageEmbed = new EmbedBuilder();
            const clonedChannel = guild.channels.cache.get(newChannelId);
            interaction.reply({embeds: [],content: `The channel has been cloned with the name of: \`\`\`${Converter(interaction.channel.name.substring(1))}\`\`\` \n Here is a link to the channel: https://discord.com/channels/${guild.id}/${newChannelId}`, ephemeral: true});
            return
        })
    }
    ...
    if (interaction.commandName === 'ping') {
        ...
        await interaction.reply( {content: '...', fetchReply: true }).then (async (resultinteraction) =>{
            ...
            await interaction.editReply({ content: '', embeds: [pong] , ephemeral: false });
        })
    }
    if (interaction.commandName === 'channelinfo') {
        ...
        newEmbed.addFields(
            ...
            {name: '📚 Catagory:', value: `${interaction.channel.parent.name}`, inline: true},
            {name: '📁 Topic:', value: `${interaction.channel.topic}`, inline: true},
            {name: '🧭 Postion in catagory:', value: `${interaction.channel.position + 1}`, inline: true},
            {name: '📚 🧭 Catagory Postion:', value: `${interaction.channel.parent.position + 1}`, inline: true},
    ...
    if (interaction.commandName === 'botinfo') {
        ...
        newEmbed.addFields(
            ...
            {name: '🎨 Creator:', value: `${appOwner.owner.tag}`, inline: true},
            {name: '💾 Servers:', value: `${client.guilds.cache.size}`, inline: true},
    ...
    const brokenCommands = ['feetpics', 'catagoryinfo', 'userinfo'];
    ...
    if (interaction.commandName === 'help') {
        ...
        helpCommand.setTitle('Bot commands:')
        ...
        interaction.reply({ embeds: [helpCommand] , ephemeral: false })
        return 
    }
    if (interaction.commandName === 'smashorpass'|| interaction.customId ==='smash'|| interaction.customId ==='pass'){
        const fs = require('fs');
        const path = require('path');
        const folderPath = 'images/people';
        const numbersFilePath = './vote.txt';
        const numbersFilePath2 = './vote2.txt';
        ...
        fs.readdir(folderPath, (err, files) => {
            ...
            fs.readFile(numbersFilePath, 'utf8', (err, data) => {
                ...
                if (numbers.length !== imageFiles.length) {
                  console.error('Number of images and numbers do not match');
                  return;
                }
                fs.readFile(numbersFilePath2, 'utf8', (err, data) => {
                    ...
                    if (interaction.customId ==='smash') {
                        ...
                        embedmeme.setColor('Pink')
                        embedmeme.setTitle(people[person].filename)
                        ...
                        interaction.update({ embeds: [ embedmeme ], files: ['./images/people/' + people[person].filename], components: [row] })
                    ...
                    if (interaction.commandName === 'smashorpass'){
                        ...
                        embedmeme.setImage('attachment://' + encodeURIComponent(people[person].filename))
                        embedmeme.setDescription("Smashes: " + people[person].number + '\nPasses: '+ people[person].number2)
                        interaction.reply({ embeds: [ embedmeme ], files: ['./images/people/' + people[person].filename], components: [row]});  
                    }
                })
            })
        });
