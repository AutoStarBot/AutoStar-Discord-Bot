...
module.exports = {
    name: 'proceedsfwcosplay',
    cooldown: 5,
    async execute(interaction) {
        try {
            const folderPath = 'smash/CosplaySFW';
            const numbersFilePath = 'smash/sfwcosplaysmash.txt';
            const numbersFilePath2 = 'smash/sfwcosplaypass.txt';
            ...
            if (numbers.length < imageFiles.length) {
                ...
                console.log(chalk.magenta('sfwcosplaysmash.txt updated successfully.'));
            }
            ...
            if (numbers2.length < imageFiles.length) {
                ...
                console.log(chalk.magenta('sfwcosplaypass.txt updated successfully.'));
            }
            ...
            const row = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        ...
                        .setStyle('Secondary'),
                    new ButtonBuilder()
                        .setCustomId('passcosplaysfw')
                        .setLabel('Pass')
                        .setStyle('Danger'),
                    new ButtonBuilder()
                        .setCustomId('smashcosplaysfw')
                        .setLabel('Smash')
                        .setStyle('Success'),
                    new ButtonBuilder()
            ...
            const selectedPerson = people[person];

            // Create embed message
            const embedmeme = new EmbedBuilder()
                .setColor('#FFC0CB')
                .setTitle(selectedPerson.filename.slice(4).replace(/\..*/, '').replace(/[-_]/g, ' ') + ' I SFW Cosplay')
                .setImage('attachment://' + encodeURIComponent(selectedPerson.filename))
                .setDescription(`Smashes: ${selectedPerson.number}\nPasses: ${selectedPerson.number2}`)
                .setFooter({ text: selectedPerson.filename });

            // Update the interaction with the new embed and buttons
            await interaction.update({
                embeds: [embedmeme],
                files: [`${folderPath}/${selectedPerson.filename}`],
                components: [row]
            });
